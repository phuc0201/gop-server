...
} from './entities/restaurant_category.schema';
...
export class RestaurantCategoryService extends BaseServiceAbstract<RestaurantCategory> {
  constructor(
    ...
    private readonly restaurantCategoryModel: Model<RestaurantCategory>,
  ...
  async updateImage(id: string, image: string) {
    await this.restaurantCategoryModel
      .findByIdAndUpdate(
        id,
        {
          image: image,
  ...
  async addFoodItem(food_item_id: string, category_id: string) {
    return await this.restaurantCategoryModel.findByIdAndUpdate(
      category_id,
      {
        $push: {
          food_items: food_item_id,
  ...
  async getMenuDetails(id: string) {
    const cate = await this.restaurantCategoryModel
      .findById(id)
      .select('name')
      .populate({
        path: 'food_items',
  ...
  async deleteFoodItem(category_id: string, food_id: string) {
    ...
    const newFoodItems = category.food_items.filter(
      (itemId) => itemId !== food_id,
    ) as string[];

    await this.restaurantCategoryModel.findByIdAndUpdate(
      category_id,
      {
