...
export class FoodItemService extends BaseServiceAbstract<FoodItem> {
  ...
  async createFoodItem(dto: CreateFoodItemDto) {
    const mg = await this.modiferGroupService.createModifieriGroupList(
      dto.modifier_groups as ModifierGroup[],
    );
  ...
  async updateFoodItem(foodItemDto: UpdateFoodItemDto) {
    ...
    if (!foodItem) {
      ...
      const currentModifierGroupIds = (
        foodItem.modifier_groups as string[]
      ).map((groupId) => groupId.toString());
      const updatedModifierGroupIds = (
        foodItemDto.modifier_groups as ModifierGroupsDto[]
      ).map((group) => group._id);
      const modifierGroupsToDelete = currentModifierGroupIds.filter(
        (id) => !updatedModifierGroupIds.includes(id),
      );

      this.modiferGroupService.deleteModifierGroups(modifierGroupsToDelete);

      const newMdGroups = await this.modiferGroupService.updateModifierGroups(
        foodItemDto.modifier_groups as ModifierGroupsDto[],
      );
  ...
  async updateFoodItemImg(id: string, imgUrl: string) {
    await this.foodItemModel.findByIdAndUpdate(
      id,
      {
        image: imgUrl,
      },
  ...
  async getFoodImagesAndNames(iDs: string[]) {
    const foodDetails = await this.foodItemModel
      .find({
        _id: { $in: iDs },
      })
