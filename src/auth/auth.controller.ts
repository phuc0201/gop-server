...
export class AuthController {
  ...
  private sign_up(
    ...
    res: Response,
    service: AccountServiceAbstract<Account>,
  ) {
    service
      .signUp(body)
      .then(async (account) => {
  ...
  private async verifyOTP(
    ...
    service
      .findOneByCondition({ email: body.email })
      .then(async (account) => {
        if (!account) {
          return res
            .status(HttpStatus.NOT_FOUND)
        ...
        const otp_result = await this.accountService.verifyOtp(
          account._id,
          body.otp,
  ...
  private async refreshToken(
    ...
    const token = await service.findOneByCondition({
      _id: req.user.sub,
    ...
    if (!token) {
      return res
        ...
        .json({ message: 'Refresh token is incorrect, please login again' });
    }
    const new_token = await this.accountService.getTokens(
      req.user.sub,
      service.name,
    );
  ...
  // todo
  // * Forgot password
  private async forgotPasswordRequest(
    email: string,
    res: Response,
    service: AccountServiceAbstract<Account>,
  ) {
    service
      .findOneByCondition({ email: email })
      .then(async (account) => {
        if (!account) {
          return res
            .status(HttpStatus.NOT_FOUND)
        ...
        this.accountService.sendOTPMail(
          account.email,
          account.full_name,
          account._id,
  ...
  private async forgotPasswordValidate(
    body: OtpVerifyDto,
    res: Response,
    service: AccountServiceAbstract<Account>,
  ) {
  ...
  async forgotPasswordRequest_customer(
  ...
  async forgotPasswordValidate_customer(
  ...
  async forgotPasswordRequest_driver(
  ...
  async forgotPasswordValidate_driver(
    @Body() body: OtpVerifyDto,
    @Res() res: Response,
  ) {
  ...
  async forgotPasswordRequest_restaurant(
    @Body('email') email: string,
    @Res() res: Response,
  ) {
  ...
  async forgotPasswordValidate_restaurant(
    @Body() body: OtpVerifyDto,
    @Res() res: Response,
  ) {
