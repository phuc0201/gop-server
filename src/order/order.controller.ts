...
export class OrderController {
  ...
  async handleOrderStatusChangeEvent(
    payload: TransportOrderType,
    status: OrderStatus,
  ) {
  ...
  async arriving(req: RequestWithUser, @Param('id') dto: any) {
    return this.orderService.orderDriverCheckAndChangeStatus(
      req.user.sub,
      dto,
  ...
  async driverGetOrder(@Req() req: RequestWithUser, dto: any) {
    return await this.orderService
      .findAll({ dirver: req.user.sub, ...dto })
      .then((order) => {
        return order;
      })
      .catch((e) => {
  ...
  rejectOrderRestaurant(
    @Req() req: RequestWithUser,
    @Body() body: { cancel_reason: string; id: string },
  ): Promise<any> {
    try {
      const res = this.orderService.RestaurantRejectOrder(
        body.id,
        body.cancel_reason,
      );
      return res;
