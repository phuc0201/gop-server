...
import {
  TransportOrder,
  TransportOrderType,
} from './entities/transport_order.schema';
import {
  DeliveryOrder,
  DeliveryOrderType,
} from './entities/delivery_order.schema';
...
export class OrderService extends BaseServiceAbstract<OrderDetails> {
  constructor(
    ...
    @InjectModel(TransportOrder.name)
    private readonly transportOrderModel: Model<TransportOrderType>,
    @InjectModel(DeliveryOrder.name)
    private readonly deliveryOrderModel: Model<DeliveryOrderType>,
  ...
  async TransportOrderQuote(
    dto: CreateTransportOrderDto,
  ): Promise<TransportOrderType> {
    const new_transport_order = new this.transportOrderModel(dto);

    let { distance, duration } = await this.vietMapService.getDistanceNDuration(
      dto.pickup_location,
  ...
  async orderDriverCheckAndChangeStatus(
    id: string,
    driver_id: string,
    status: OrderStatus,
  ): Promise<OrderDetails> {
